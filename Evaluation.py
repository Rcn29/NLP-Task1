...
if get_platform() == 'Windows':
    learn_file = os.path.join(os.path.join(
        os.getcwd(), 'SVM-Light'), 'svm_learn_windows')
    classify_file = os.path.join(os.path.join(
        os.getcwd(), 'SVM-Light'), 'svm_classify_windows')
elif get_platform() == 'Linux':
    learn_file = os.path.join(os.path.join(
        os.getcwd(), 'SVM-Light'), 'svm_learn_linux')
    classify_file = os.path.join(os.path.join(
        os.getcwd(), 'SVM-Light'), 'svm_classify_linux')
elif get_platform() == 'OS X':
    learn_file = os.path.join(os.path.join(
        os.getcwd(), 'SVM-Light'), 'svm_learn_osx')
    classify_file = os.path.join(os.path.join(
        os.getcwd(), 'SVM-Light'), 'svm_classify_osx')
...
def cross_validation(model, smoothed, features, mode, path_pos_reviews, path_neg_reviews, stemmed, no_of_splits, file):
    pos_reviews = create_review_list(path_pos_reviews, stemmed)
    neg_reviews = create_review_list(path_neg_reviews, stemmed)
    ...
    reviews_without_validaton = []

    for it in range(0, len(splits_with_validation) - 1):
        for name in splits_with_validation[it]:
            reviews_without_validaton.append(name)

    splits = stratified_split(reviews_without_validaton, no_of_splits)
    ...
    for it in range(0, len(splits)):
        training_set = generate_training_set(splits, it)
        test_set = generate_test_set(splits, it)

        if model == "Naive Bayes":

            if features == "Unigrams":
                unigram_probs = generate_unigram_probs(
                    training_set, original_labels, smoothed)
                unigram_feature_vectors = compute_unigram_feature_vectors(
                    test_set, unigrams, True, mode)
                predicted_labels = naivebayes(
                    unigram_probs, unigram_feature_vectors, test_set)

            elif features == "Bigrams":
                bigram_probs = generate_bigram_probs(
                    training_set, original_labels, smoothed)
                bigram_feature_vectors = compute_bigram_feature_vectors(
                    test_set, bigrams, True, mode)
                predicted_labels = naivebayes(
                    bigram_probs, bigram_feature_vectors, test_set)

            elif features == "Both":
                both_probs = generate_both_probs(
                    training_set, original_labels, smoothed)
                unigram_feature_vectors = compute_unigram_feature_vectors(
                    test_set, unigrams, True, mode)
                bigram_feature_vectors = compute_bigram_feature_vectors(
                ...
                predicted_labels = naivebayes(
                    both_probs, feature_vectors, test_set)

        elif model == "SVM":
            predicted_labels = {}

            if features == "Unigrams":
                training_unigram_feature_vectors = compute_unigram_feature_vectors(
                    training_set, unigrams, True, mode)
                test_unigram_feature_vectors = compute_unigram_feature_vectors(
                    test_set, unigrams, True, mode)
                ...
                training_bigram_feature_vectors = compute_bigram_feature_vectors(
                    training_set, bigrams, True, mode)
                test_bigram_feature_vectors = compute_bigram_feature_vectors(
                    test_set, bigrams, True, mode)
                ...
                for bigram in bigrams_counter:
                    bigrams_counter[bigram] += len(unigrams_counter)
					
                both_counter = merge_dictionaries(
                    unigrams_counter, bigrams_counter)

                process_feature_vectors_to_svm_input(
                    training_feature_vectors, both_counter, original_labels, "train")
                process_feature_vectors_to_svm_input(
                    test_feature_vectors, both_counter, original_labels, "test")
			
			#-z c is setting for classification in SVM light
            subprocess.call(learn_file + " -z c  " +
                            train_file + " " + model_file, shell=True)
            subprocess.call(classify_file + " " + test_file +
                            " " + model_file + " " + results_file, shell=True)
        ...
        if it == 0:
            all_labels.update(predicted_labels.copy())
